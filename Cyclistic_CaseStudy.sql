...
SELECT
	member_casual,
	AVG(ride_length_seconds)/60 as avg_ride_length
FROM
	[PortfolioProjects].[dbo].[Cyclistic_TripData_2022]
GROUP BY 
...
--(2)Calculating the average ride length monthly by each group
SELECT
	member_casual,
	month,
	AVG(ride_length_seconds)/60 as avg_ride_length
...
FROM
	[PortfolioProjects].[dbo].[Cyclistic_TripData_2022]
GROUP BY member_casual --694705.128888889min casual total ride length  "56.2%"
					   --541351.7175min member total ride length       "43.8"
...
--Calculating total number of rides by hour daily for each group
SELECT
	member_casual,
	day_of_week,
	hour_of_day,
...
--Calculating total number of rides by hour for each group
SELECT
	member_casual,
	hour_of_day,
	COUNT(member_casual)
